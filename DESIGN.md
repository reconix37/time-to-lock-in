...
- **Дерево категорий (v0.2.9)**: строки высотой не менее 48px, вложение по сетке
  20px на уровень, максимум три уровня. В строке: disclosure-кнопка, effective-color
  dot, имя, score и краткое описание правил. Положительный score — mono 500 `--ok`
  с явным `+`, отрицательный — mono 500 `--bad`, нулевой — `--muted`. Полные пути
  всегда пишутся через ` > `; обрезка допустима только визуально с полным путём в title.
- **Score card (v0.2.9)**: `--surface`, `--border`, radius 10, padding 20px. Total score
  — hero 32px/700 mono (`--ok` при >0, `--bad` при <0, `--muted` при 0), productive%
  — mono 13px `--muted`. Top productive и Top distracting — компактные списки с
  полным путём и score-hours; новые цвета не вводятся.
...
- Печать дня — порядок строк (канон): `STATUS: <вердикт> · +<XP>` → три условия зачёта (✓/✕, три колонки на широком экране / одна на узком) → challenge (если был) → короткая сводка (neutral, AFK, burned ₽; useful/waste не дублируются — они уже в условиях) → топ-5 приложений с длительностью → privacy footer («Tracked locally. No screenshots.») → кнопки экспорта. Заголовок `TIMEFORGE // DAILY LOG — <дата>` + выбор даты (всегда выбранная дата, без автоподмены; пустой день — «НЕТ ДАННЫХ» без красного наказания). Токены: `--term-bg` (#2a2534 Dawn / #1f1c2b Moon), `--term-text`, `--term-muted` — уже определены в темах
