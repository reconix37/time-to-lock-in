...
function ConditionLine({ label, value, threshold, passed, limit = false }: {
  label: string;
  value: number;
  threshold: number;
  passed: boolean;
  ...
  return (
    <div className="day-print-condition">
      <span className={passed ? "is-ok" : "is-bad"} aria-label={passed ? t("print.conditionComplete") : t("print.conditionIncomplete")}>{passed ? "✓" : "×"}</span>
      <strong>{label}</strong>
    ...
    <section className="day-print-section" aria-labelledby="day-print-title">
      ...
      <div className="day-print-terminal">
        ...
        <div className="day-print-conditions">
          <ConditionLine label={kindLabels.useful.toLocaleUpperCase(locale)} value={data.useful_ms} threshold={data.useful_goal_min} passed={data.useful_passed} />
          <ConditionLine label={kindLabels.waste.toLocaleUpperCase(locale)} value={data.waste_ms} threshold={data.waste_limit_min} passed={data.waste_passed} limit />
          <ConditionLine label={(observedLabel ?? kindLabels.observed).toLocaleUpperCase(locale)} value={data.observed_ms} threshold={data.observed_min} passed={data.observed_passed} />
        </div>
        <p className={`day-print-status ${data.passed ? "is-ok" : "is-bad"}`}>
          {t("print.status")} {data.passed ? t("print.statusPassed") : t("print.statusFailed")} · +{data.public_xp} XP · {data.rank}
        </p>
        {data.challenge_passed !== null && (
          <p className={`day-print-challenge ${data.challenge_passed ? "is-ok" : "is-bad"}`}>
            {data.challenge_passed ? t("print.challengePassed") : t("print.challengeFailed")} · {data.challenge_code}
          </p>
        )}
