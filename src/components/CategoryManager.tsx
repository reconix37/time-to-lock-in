          ...
          {loading ? <div className="manager-loading skeleton" /> : tab === "categories" ? (
            ...
            <div role="tabpanel">
              ...
              <form className="manager-form rule-edit-form" onSubmit={(event) => { event.preventDefault(); void saveRule(newRule); }}>
                <label className="manager-field"><span>{t("manager.type")}</span><select value={newRule.match_type} onChange={(event) => setNewRule({ ...newRule, match_type: event.target.value as RuleMatchType })}><option value="exe">exe</option><option value="title">title</option><option value="domain">domain</option></select></label>
                <label className="manager-field"><span>{t("manager.regexMode")}</span><select value={newRule.match_mode} onChange={(event) => setNewRule({ ...newRule, match_mode: event.target.value as RuleMatchMode })}><option value="legacy">legacy</option><option value="regex">regex</option></select></label>
                ...
                <label className="manager-field"><span>{t("classification.category")}</span><select value={newRule.category_id} onChange={(event) => setNewRule({ ...newRule, category_id: Number(event.target.value) })}>{manageable.map((category) => <option key={category.id} value={category.id}>{category.full_path}</option>)}</select></label>
              ...
              <div className="manager-list rule-list">{rules.map((rule) => <div className="rule-manager-row rule-manager-edit" key={rule.id}>
                <select value={rule.match_mode} onChange={(event) => void saveRule({ ...rule, match_mode: event.target.value as RuleMatchMode }, rule.id)}><option value="legacy">legacy</option><option value="regex">regex</option></select>
                ...
                <select value={rule.category_id} onChange={(event) => void saveRule({ ...rule, category_id: Number(event.target.value) }, rule.id)}>{manageable.map((category) => <option key={category.id} value={category.id}>{category.full_path}</option>)}</select>
