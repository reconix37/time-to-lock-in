...
export const CATEGORY_ICONS: Record<string, JSX.Element> = {
  ...
  "heart": <path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.51 4.05 3 5.5l7 7Z" />,
...
export function CategoryIcon({ icon, size = 16, color }: CategoryIconProps) {
  if (!icon) return null;
  const paths = CATEGORY_ICONS[icon] ?? CATEGORY_ICONS.tag;
  return <svg className="category-icon" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={color ? { color } : undefined} aria-hidden="true">{paths}</svg>;
}

export function CategoryMark({ icon, color, compact = false }: CategoryIconProps & { compact?: boolean }) {
  if (compact && icon) return <CategoryIcon icon={icon} color={color} />;
  return <span className="category-mark" style={{ "--category-color": color } as CSSProperties}><span className="manager-color-dot" />{icon && <CategoryIcon icon={icon} />}</span>;
}
